...
import GlobalSettingsPanel from "@/components/gantt/GlobalSettingsPanel";
import { loadDisplaySettings, saveDisplaySettings, applyWbsGrouping, wbsLevelCounts } from "@/lib/displaySettings";
...
export default function GanttPage() {
  ...
  // Quick filters (batch 20) — like displaySettings, this must be declared BEFORE
  // the derived-task block below, which reads it (same TDZ trap as batch 8.1).
  ...
  // (XER `PROJECT.last_recalc_date`). The status pills are read as at this date,
  // so it must also be declared BEFORE the derived block.
  ...
  // Quick filters (batch 20/21) — status pills / started / milestones / critical
  // path, read as at the record's Last Recalc Date. Group headers with no
  // surviving activity are dropped (like "Diff Only"); the activities themselves
  ...
  // "Customise Grouping" (WBS Settings, batch 19) — group the list by WBS down to
  // the chosen level, or turn the WBS grouping off entirely (flat activity list).
  // Activity rows are untouched; only the group-header rows are affected.
  filtered = applyWbsGrouping(filtered, displaySettings?.grouping);

  // WBS level inventory for the settings panel's "Customise Grouping" section —
  // the reference panel lists only the levels that exist in the loaded file.
  ...
  useEffect(() => {
    if (!xerSource) return;
    setLastRecalcDate(recalcDateFromXerTables(xerSource));
  }, [xerSource]);

  // Manual edit in the Quick Filter menu — kept until another programme is loaded.
  const handleRecalcDateChange = useCallback((value) => {
    setLastRecalcDate(normalizeRecalcDate(value));
  }, []);
  ...
  const otherExtras = (
    <div className="mt-3 flex flex-col gap-2">
      {showRelationFilter && (
        <div className="flex items-center gap-1 border-t border-border pt-2">
          ...
          {["FS", "SS", "FF", "SF"].map(tp => (
            <button
              key={tp}
              onClick={() => setRelationTypeFilter(prev => ({ ...prev, [tp]: !prev[tp] }))}
              className={`px-1.5 py-0.5 rounded text-xs font-bold border transition-all ${
                relationTypeFilter[tp]
          ...
          <button
            onClick={() => setRelationTypeFilter({ FS: true, SS: true, FF: true, SF: true })}
      ...
      {selectedIds && selectedIds.size > 0 && (
        ...
        >
          Clear selection ({selectedIds.size}) <span className="text-text-muted">· Esc</span>
        </button>
      )}
      <div className="border-t border-border pt-2">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-semibold text-text">Font Size</span>
          <button onClick={() => setTableFontScale(1.0)} className="text-xs text-primary hover:underline">Reset</button>
        </div>
        <input
        ...
        <div className="flex justify-between text-xs text-text-muted mt-0.5">
          <span>70%</span><span className="font-medium text-text-muted">{Math.round(tableFontScale * 100)}%</span><span>150%</span>
        </div>
      </div>
    </div>
  );

  const viewPresetsNode = (
    <div className="p-4 border-b border-border">
      <ViewPresets columnVisibility={columnVisibility} onApply={(vis) => setColumnVisibility(vis)} />
    </div>
  );
    ...
    <div className="relative h-screen bg-surface-subtle flex flex-col overflow-hidden">
      ...
      <div ref={headerRef} className="flex-shrink-0 bg-primary-dark text-surface px-6 py-3 flex items-center justify-between shadow-md z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* ── Quick filters (status / started / milestones / critical path) ──
                 Single filter entry point: the funnel opens the pills plus the
                 "Last Recalc Date" the statuses are read as at; "Custom" opens the
                 full P6 filter dialog. (The Tools menu no longer duplicates it.) ── */}
          <QuickFilterMenu
            quickFilters={quickFilters}
            onChange={setQuickFilters}
            customFilter={ganttFilters}
            onOpenCustom={() => setShowFilter(true)}
          ...
          {/* ── Tools (collapsed) ──
                 NOTE (batch 21): the old "P6 Filters" entry was removed — the
                 toolbar funnel menu is the single filter entry point (pills +
                 Last Recalc Date + Custom → filter dialog + Clear). ── */}
          <HeaderDropdown
            icon={Settings}
            label="Tools"
          >
      ...
      {/* Filter dialog */}
      <FilterDialog open={showFilter} onClose={() => setShowFilter(false)} filters={ganttFilters} onFiltersChange={setGanttFilters} recalcDate={lastRecalcDate} />
      ...
      {/* Unified settings panel — xerviewer-style rail: WBS / Activity List /
          Timeline & Grid / Gantt Bars / Other. Replaces the old Gantt Settings
          and WBS Settings modals and the header "Display" dropdown. */}
      {showGlobalSettings && (
        <GlobalSettingsPanel
